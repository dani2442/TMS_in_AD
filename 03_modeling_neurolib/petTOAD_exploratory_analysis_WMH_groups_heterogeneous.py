...
ws = np.linspace(ws_min, ws_max, 11)
bs = np.linspace(bs_min, bs_max, 11)
...
if not random:
    EXPL_DIR = RES_DIR / f"exploratory_heterogeneous_ws_{ws_min}-{ws_max}_bs_{bs_min}-{bs_max}"
else:
    EXPL_DIR = RES_DIR / f"exploratory_heterogeneous_ws_{ws_min}-{ws_max}_bs_{bs_min}-{bs_max}_random"
