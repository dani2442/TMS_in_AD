...
def simulate_heterogeneous_model(
    subj, wmh_subjs, f_diff, best_G, ws, bs, random_cond, sim_dir, nsim
):
    global search
    print(f"Now performing the simulations for the heterogeneous model...")
    if not random_cond:
        node_damage = get_node_damage(subj, wmh_subjs, is_random=False)
    else:
        node_damage = get_node_damage(subj, wmh_subjs, is_random = True)
...
def prepare_subject_simulation_disconn(subj, wmh_subjs, model, ws, random_cond):
    sc = model.params.Cmat
    damage_sc = get_sc_wmh_weighted(subj, wmh_subjs, is_random = random_cond)
    # Define the parametere space to explore
    parameters = ParameterSpace(
        {
...
def simulate_disconn_model(subj, wmh_subjs, f_diff, best_G, ws, random_cond, sim_dir, nsim):
    global search
    ...
    savedir = str(sim_dir)
    paths.HDF_DIR = savedir
    parameters, filename = prepare_subject_simulation_disconn(subj, wmh_subjs, model, ws, random_cond)
    for i in range(nsim):
        print(f"Starting simulations n°: {i+1}/{nsim}")
        # Initialize the search
        search = BoxSearch(
...
if __name__ == "__main__":
    ...
    SIM_DIR = RES_DIR / "simulations_2024_06_18"
