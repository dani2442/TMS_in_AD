...
for subj_n, subj in enumerate(sim.short_subjs):
    ...
    if not random_condition:
        ...
        filename = f"{sim.paths.HDF_DIR}/{subj}_heterogeneous_model_random.hdf"
        trajs = pu.getTrajectorynamesInFile(f"{sim.paths.HDF_DIR}/{subj}_heterogeneous_model_random.hdf")
    big_list = []
    for traj in trajs:
        traj_list = []
...
#%%    
    ...
    nsim = len(trajs)
    nparms = len([np.array([round(-0.02 - w*(1-n) + b, 5) for n in np.arange(90)]) for w in np.linspace(sim.ws_min, sim.ws_max, sim.ws_n) for b in np.linspace(sim.bs_min, sim.bs_max, sim.bs_n)])
