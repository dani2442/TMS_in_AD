...
import petTOAD_exploratory_analysis_WMH_groups_delay_matrix as sim
...
def calculate_results_from_bolds(bold_arr):
    ...
    phFCD_length = len(my_func.phFCD(bold_arr[0,0])) # I realize this might not be the best way... but we do it only once per subj so it's not too time consuming...
    phfcd_array = np.zeros([nsim, nparms, phFCD_length])
...
for subj_n, subj in enumerate(sim.short_subjs):
    ...
    filename = f"{sim.paths.HDF_DIR}/{subj}_model_delay_matrix.hdf"
