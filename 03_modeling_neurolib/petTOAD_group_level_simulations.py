...
def prepare_parms(group, grouplist, model_type, G):
    ...
    #     node_damage = get_group_node_damage(
    #         grouplist, is_random=False
    #     )  # now placeholder for testing, should become load_node_damage_group(group)
    #     parameters = ParameterSpace(
    #         {
    #             "a": [
    ...
    # elif model_type == "heterogeneous_rand":
    #     node_damage = get_group_node_damage(
    #         grouplist, is_random=True
    #     )  # now placeholder for testing, should become load_node_damage_group(group)
    #     parameters = ParameterSpace(
    #         {
    #             "a": [
    ...
    #     spared_sc = get_group_sc_wmh_weighted(
    #         grouplist
    #     )  
    #     parameters = ParameterSpace(
    #         {
...
# def create_df_results_het(fc_pearson, phfcd_ks, savedir, group, is_rand):
#     data = [
...
#     if not is_rand:
#         res_df.to_csv(f"{savedir}/group-{group}_model-heterogeneous_desc-df-fitting-results.csv")
#     else:
#         res_df.to_csv(f"{savedir}/group-{group}_model-heterogeneous-rand_desc-df-fitting-results.csv")

# def create_df_results_sc_disconn(fc_pearson, phfcd_ks, savedir, group):
#     data = [[[w for w in ws_disconn], fc_pearson, phfcd_ks]]
#     columns = ["w", "fc_pearson", "phfcd_ks"]
#     res_df = pd.DataFrame(data, columns=columns)
#     res_df.to_csv(f"{savedir}/group-{group}_model-disconn_desc-df-fitting-results.csv")
...
def create_df_results(model_type, parameters, fc_pearson, phfcd_ks, savedir, group):
    ...
    # elif model_type == "heterogeneous":
    #     create_df_results_het(fc_pearson, phfcd_ks, savedir, group, is_rand = False)
    #     return
    
    # elif model_type == "heterogeneous_rand":
    #     create_df_results_het(fc_pearson, phfcd_ks, savedir, group, is_rand = True)
    #     return
    ...
    # elif model_type == "disconn":
    #     create_df_results_sc_disconn(fc_pearson, phfcd_ks, savedir, group)
    #     return


# def save_plot_results_fitting_parm(parm_name, res_df, group):
#     dict_x_lab = {"a": "Bifurcation parameter (a)", "K_gl": "Global coupling (G)"}
#     if parm_name == "a":
...
def simulate_group(group, grouplist, fdiff, G, nsim, model_type):
    ...
    parameters, filename = prepare_parms(group, grouplist, model_type, G)
    # for i in range(nsim):
...
model_types_per_group = {
    ...
    "CN_WMH": ["homogeneous_a", "homogeneous_G"], #"homogeneous_a", "homogeneous_G", "disconn",
    "MCI_WMH": ["homogeneous_a","homogeneous_G"], #"homogeneous_a", "homogeneous_G", "disconn",
}

# ##############################################################################################
# ########################### Parameters for the heterogeneous model ###########################
# ##############################################################################################
...
# ###############################################################################################
# ########################## Parameters for the dysconnectivity model ###########################
# ###############################################################################################
# # Set the exploration values for the heterogeneous model
# ws_min_disconn = 0.0
# ws_max_disconn = 1.0
