...
@loadOrCompute
# Simulate the model with the parameters of that part of the for loop and
# returns the metrics we want.
def distanceForOne_Parm(currValue, modelParms, NumSimSubjects,
    ...
    current_a = integrator.neuronalModel.getParm('a')
    print(f'Current a is: {current_a}')
    current_G = integrator.neuronalModel.getParm('we')
    print(f'Current G is: {current_G}')
...
def distanceForAll_Parms(tc, #dictionary where all patients timeseries are stored
                         Parms,  # wStart=0.0, wEnd=6.0, wStep=0.05, list of parameters to explore
                         modelParms, # other parameters of the model that we are not going to explore
                         NumSimSubjects, #number of simulated subjects
                         distanceSettings,  # This is a dictionary of {name: (distance module, apply filters bool)}
                         parmLabel='', #just a convenience parameter to explicit what we are doing. E.g., "linear" if we are doing linear regression on a depending on tau volumes
                         outFilePath=None, # where to save output
                         fileNameSuffix=''):
    ...
    for pos, parm in enumerate(np.nditer(Parms)):  # iteration over the values for G (we in this code)
        ...
        simMeasures = distanceForOne_Parm(parm, modelParms[pos], NumSimSubjects,
                                          ...
                                          outFileNamePattern.format(np.round(parm, decimals=5)))
    ...
    # for ds in distanceSettings:
    #      plt.plot(Parms, fitting[ds], label = ds)
    #      plt.legend()
    ...
    # plt.savefig(outFilePath + f'/plot_for_{parmLabel}_{fileNameSuffix}.png')
    print("#####################################################################################################\n\n")
